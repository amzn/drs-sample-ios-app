...
#import "AMZNUIWebViewDelegate.h"
#import "AMZNUIWebViewController.h"
#import "Constants.h"

@interface AMZNUIWebViewDelegate ()

@property(nonatomic) AMZNUIWebViewController *webViewController;

@end

#pragma mark -

@implementation AMZNUIWebViewDelegate {

}

#pragma mark - Initialization

- (instancetype)initWithWebViewController:(AMZNUIWebViewController *)webViewController {
    self = [super init];
    if (self) {
...
- (BOOL)webView:(UIWebView *)webView shouldStartLoadWithRequest:(NSURLRequest *)request
 navigationType:(UIWebViewNavigationType)shouldStartLoadWithRequest {
    //Intercept URL load request and execute proper action.
    if (UIWebViewNavigationTypeLinkClicked == shouldStartLoadWithRequest) {
        NSString *key = [self getKey:request];
        if ([key isEqualToString:REMINDER_URI_VALUE]) {
            [self loadPage:key];
            return NO;
        } else if ([key isEqualToString:SKIP_URI_VALUE]) {
            [self loadPage:key];
            return NO;
        } else if ([key isEqualToString:LOGIN_URI_VALUE]) {
            [self loginUser];
            return NO;
        } else if ([key isEqualToString:BACK_URI_VALUE]) {
            [self navigateBack];
            return NO;
        }
        return YES;
...
/**
 * Parse the URL from the request and returns the last part of the URL.
...
- (NSString *)getKey:(NSURLRequest *)request {
    ...
    if ([urlComponents count] != 0) {
        return urlComponents[[urlComponents count] - 1];
    }
    return [NSString string];
}

/**
 * Login the user. Proceed to main app view controller.
 */
- (void)loginUser {
    [self.webViewController loginUser];
}

/**
 * Load the web page from a local HTML file.
 * @param path The name of the file to load.
 */
- (void)loadPage:(NSString *)path {
    [self.webViewController displayPageForPath:path];
}
 ...
 */
- (void)navigateBack {
    [self.webViewController goToPreviousPage];
}
