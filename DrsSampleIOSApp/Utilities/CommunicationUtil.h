...
/**
 * The CommunicationUtil class calls DRS backend APIs. To call any of the backend APIs,
 * instantiate an object of this class with an object that implements DrsDeviceProtocol.
 * To successfully call backend APIs the DrsDeviceProtocol must have a valid access token.
 */
@interface CommunicationUtil : NSObject

/**
 * Initialize a communication util object with a DrsDeviceProtocol object.
 * @param drsHandler DrsDeviceProtocol.
 */
...
/**
 * Send the deregister command.
 */
- (void)deregisterDevice;

/**
 * Send the initiate replenish command.
 * @param slotID ID of the slot which we want to replenish.
 */
- (void)initiateReplenishForSlot:(NSString *)slotID;

/**
 * Send the slot status.
 * @param slotID ID of the slot for which we are sending the status.
 */
- (void)sendSlotStatusForSlot:(NSString *)slotID;

/**
 * Get the subscription info for the device.
 */
- (void)sendSubscriptionInfo;

/**
 * Cancel test oreder for the specific slot.
