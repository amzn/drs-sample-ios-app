...
@interface AMZNUIWebViewController ()

@property(nonatomic) AMZNUIWebViewDelegate *delegate;

@end

#pragma mark -

@implementation AMZNUIWebViewController

NSString *const TITLE = @"Sample Companion App";
...
- (void)viewDidLoad {
    ...
    [_webView setDelegate:self.delegate];

    //load teaser url specified in a SetupProductViewController.
...
/**
 * Get the UIWebViewDelegate.
 * @return The AMZNUIWebViewDelegate.
 */
- (AMZNUIWebViewDelegate *)getDelegate {
    return [[AMZNUIWebViewDelegate alloc] initWithWebViewController:self];
}

#pragma mark - Public methods

- (void)loginUser {
    [[self navigationController] pushViewController:[self getViewController] animated:YES];
}

- (void)displayPageForPath:(NSString *)path {
...
/**
 * Get the DRSMainViewController.
 * @return The DRSMainViewController or nil.
 */
- (DRSMainViewController *)getViewController {
    UIStoryboard *storyboard = [self storyboard];
    if ([MAIN_VIEW_CONTROLLER_CONST length] != 0) {
        DRSMainViewController *viewController = [storyboard instantiateViewControllerWithIdentifier:MAIN_VIEW_CONTROLLER_CONST];
